    ...
    def getContent(self, num):
        ...
        for i in result_even:
            ...
            if self.isAlive(t1[0], t1[1]):
                ...
                self.insert_db(t1[0],t1[1])
        for i in result_odd:
            t2 = i.xpath("./td/text()")[:2]
            print "IP:%s\tPort:%s" % (t2[0], t2[1])
            if self.isAlive(t2[0], t2[1]):
                self.insert_db(t2[0],t2[1])


    def insert_db(self,ip,port):
        dbname=self.dbname
        try:
        ...
        create_tb='''
        create table if not exists proxy(ID INTEGER PRIMARY KEY AUTOINCREMENT,IP TEXT,PORT TEXT);
        '''
